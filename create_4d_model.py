...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-c", "--cfg_file", help="Path to YOLOv5 config file", default="models/yolov5m.yaml", required=True
    )
    parser.add_argument(
        "-w", "--weights", help="Path to pretrained yolo model weights in .pt", default="yolov5m.pt", required=True
    )
    parser.add_argument(
        "-s", "--save_file", help="Path to save the new yolo pt model", default="yolov5m-4d.pt", required=True
    )
